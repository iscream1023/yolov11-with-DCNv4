...
class Bottleneck_DCN(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        if k[1] > 1:
            ...
            self.cv2 = DCN(c_, c2, k=k[1], s=1, g=g)
        else:
...
class C3k_DCN(C3k):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.m = nn.Sequential(*(Bottleneck_DCNv2(c_, c_, shortcut, g, k=(3, 3), e=1.0) for _ in range(n)))
...
class Bottleneck_DCNv4(nn.Module):
    """DCNv4를 적용한 최적화된 Bottleneck 블록 (YOLOv11용)"""
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels (예: 96)
        self.cv1 = nn.Conv2d(c1, c_, 1, 1) # 표준 1x1 Conv
        
        # DCNv4 설정: 96채널일 때 group=6을 주어 d_per_group=16을 맞춤
        # 이 설정이 0.54ms의 CPU 오버헤드를 줄이는 핵심입니다.
        if DCNv4 is not None:
            self.cv2 = DCNv4(
                ...
                stride=1, 
                pad=(k[1]-1)//2, 
                group=6 if c_ == 96 else 4 # 96채널 대응
            )
        else:
            self.cv2 = nn.Conv2d(c_, c2, k[1], 1, (k[1]-1)//2, groups=g)
    ...
    def forward(self, x):
        """
        차원 변환 흐름:
        (B, C, H, W) -> cv1 -> (B, c_, H, W) -> permute -> (B, H, W, c_) 
        -> DCNv4 -> (B, L, c_) -> view/permute -> (B, c_, H, W) -> shortcut
        """
        # 1. 첫 번째 Conv (표준 BCHW 처리)
        y = self.cv1(x)
        B, C, H, W = y.shape
        
        # 2. DCNv4를 위한 Channel-Last 변환 (B, H, W, C)
        # 이 과정이 GPU 메모리 정렬을 최적화하여 3ms 지연을 잡습니다.
        y = y.permute(0, 2, 3, 1).contiguous()
        
        # 3. DCNv4 연산 (결과값은 N, L, C 형태)
        y = self.cv2(y)
        
        # 4. YOLO 복구를 위한 차원 재조립 (N, L, C) -> (B, C, H, W)
        # L = H * W 이므로 다시 8x8 등으로 쪼개줍니다.
        y = y.view(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        
        return x + y if self.add else y
class C3k2_DCNv4(C3k2):
    """C3k2 블록의 Bottleneck을 DCN버전으로 교체"""
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        # 내부에 생성되는 Bottleneck들을 Bottleneck_DCN으로 교체
        self.m = nn.ModuleList(
            Bottleneck_DCNv4(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) if c3k else 
            Bottleneck_DCNv4(self.c, self.c, shortcut, g, k=(3, 3), e=0.5) for _ in range(n)
        )