...
from ultralytics.nn.modules.block import C3k2_DCN, C3k2_DCNv4
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            self.to('cuda')  # Move model to GPU for stride calculation
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s, device='cuda'))], device='cuda')  # forward
        ...
        if m in base_modules:
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
            #C3K2_DCN add
            if m is C3k2_DCN or m is C3k2_DCNv4:
                c1, c2 = ch[f], args[0]
            ...
            # args 리스트 재구성: [입력채널, 출력채널, 나머지인자들...]
                args = [c1, c2, *args[1:]]
                if m in (C3k2, C3k2_DCN, C3k2_DCNv4): # C3k2 계열은 n을 인자로 받음
                    args.insert(2, n)
                    n = 1
